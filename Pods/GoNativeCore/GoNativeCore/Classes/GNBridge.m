...
- (void)loadUserScriptsForContentController:(WKUserContentController *)userController {
    for(id<GNBridgeModule> moduleProvider in self->moduleProviders) {
        ...
        if (bundle == nil) { // bundle is at root for binary plugins
            bundleURL = [[[NSBundle mainBundle] resourceURL] URLByAppendingPathComponent:jsBundle];
...
- (void)application:(UIApplication *)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken {
    for(id<GNBridgeModule> moduleProvider in self->moduleProviders) {
        if([moduleProvider respondsToSelector:@selector(application:didRegisterForRemoteNotificationsWithDeviceToken:)]) {
            [moduleProvider application:application didRegisterForRemoteNotificationsWithDeviceToken:deviceToken];
        }
    }
}
...
- (void)webView:(WKWebView *)webView handleURL:(NSURL *)url {
    for(id<GNBridgeModule> moduleProvider in self->moduleProviders) {
        if ([moduleProvider respondsToSelector:@selector(webView:handleURL:)]) {
            [moduleProvider webView:webView handleURL:url];
        }
    }
}
...
- (NSArray<NSURLQueryItem *> *)getInitialUrlQueryItems {
    ...
    for(id<GNBridgeModule> moduleProvider in self->moduleProviders) {
        if ([moduleProvider respondsToSelector:@selector(getInitialUrlQueryItems)]) {
            NSArray *queryItems = [moduleProvider getInitialUrlQueryItems];
            if (queryItems) {
